...
st.markdown('<h1 class="main-title">🤖 AIZS Agent 多步推理演示</h1>', unsafe_allow_html=True)
st.markdown('<p class="sub-title">本页面为演示页，展示 Agent 的多步任务拆分与执行、多工具协同、主动澄清追问、执行轨迹展示及回答质量评估功能。</p>', unsafe_allow_html=True)

# 提示环境配置
if not settings.DASHSCOPE_API_KEY:
    st.warning("⚠️ **温馨提示：** 未配置大模型 API 密钥（DASHSCOPE_API_KEY 为空），系统将无法执行推理或检索。请在根目录创建 `.env` 并配置后再试。")
...
# 新建会话按钮
if st.sidebar.button("➕ 新建 Agent 咨询会话", use_container_width=True, type="primary"):
    try:
...
if st.session_state.agent_session_id:
    current_sess_detail = next((s for s in sessions if s["session_id"] == st.session_state.agent_session_id), None)
    if current_sess_detail:
        st.caption(f"当前 Agent 调试会话 ID：`{st.session_state.agent_session_id}`")
    ...
    for msg in messages:
        with st.chat_message(msg["role"]):
            ...
            # 若是助手消息，渲染关联的意图、轨迹、工具调用、引用来源以及质量分
                    ...
                    if srcs:
                        with sources_placeholder.expander("📖 实时匹配引用来源 (RAG Source)", expanded=False):
                            for idx, s in enumerate(srcs, 1):
                                ...
                                st.markdown(f"""
                                <div class="source-box">
                                    <strong>[{idx}] {s['file_name']} ({page_info if 'page_info' in s else page_str})</strong><br/>
