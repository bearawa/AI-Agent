...
def _render_stat_card(value, label, color="#5b86e5"):
    """渲染统计卡片。"""
    st.markdown(f"""
    <div style="background:#f8f9fa;border-radius:8px;padding:15px;
                border-top:4px solid {color};text-align:center;
                box-shadow:0 4px 6px rgba(0,0,0,0.02);">
        <div style="font-size:1.8rem;font-weight:800;color:#2b3a4a;">{value}</div>
...
def _render_import_tab(batch_service: BatchImportService):
    ...
    st.caption("分类仅用于知识库管理和检索排序辅助，不会限制用户从全知识库中查找答案。")
    st.info("💡 **推荐提示**：建议优先选择“通用资料”或“自动识别”。即使选择了具体分类，系统回答问题时也会进行全知识库检索，避免因为分类错误漏掉真实资料。")
    ...
    with tab_multi:
        ...
        if uploaded_files:
            st.info(f"已选择 **{len(uploaded_files)}** 个文件，点击下方按钮开始导入。")
            ...
            if st.button("🚀 解压并批量导入", type="primary", use_container_width=True, key="kc_zip_btn"):
                with st.spinner("正在解压并安全检验压缩包内容..."):
                    try:
                        ...
                        st.session_state["kc_import_results"] = results_list
                        st.success("✅ ZIP 压缩包处理完毕！")
                    except Exception as e:
                        st.error(f"❌ ZIP 导入出错：{e}")
                st.rerun()
    ...
    with tab_demo:
        ...
        st.info("💡 演示导入使用预定义的分类映射，不受上方分类选择框控制。")
    ...
    if st.session_state.get("kc_import_results"):
        ...
        st.markdown("---")
        st.markdown("### 📊 3. 本次导入结果汇总")

        c1, c2, c3, c4, c5 = st.columns(5)
        with c1: _render_stat_card(report["total_files"], "处理文件数")
        with c2: _render_stat_card(report["success_count"], "成功入库数", "#2ecc71")
        with c3: _render_stat_card(report["skipped_count"], "重复跳过数", "#f1c40f")
        with c4: _render_stat_card(report["failed_count"], "处理失败数", "#e74c3c")
        with c5: _render_stat_card(report["total_chunks"], "产生切片数", "#9b59b6")
...
def _render_manage_tab(admin_service: KnowledgeAdminService):
    ...
    st.markdown("---")
    st.markdown("### 📂 文档列表与批量操作")

    # 分类筛选
    categories = ["全部", "通用资料", "招生", "学务", "后勤", "校园生活", "其他"]
    col_filter, col_status_filter = st.columns([1, 1])
    with col_filter:
    ...
    col_all, col_none, col_ref = st.columns(3)
    with col_all:
    ...
    if selected_count == 0:
        ...
        if st.session_state.get("confirm_batch_delete", False):
            st.error(
                "⚠️ **高危操作确认**\n\n"
                f"该操作将删除选中的 **{selected_count}** 个文档，"
                "包括其 SQLite 记录、ChromaDB 向量切片、来源引用和上传原文件，**且不可撤销**。\n"
            ...
            col_exec, col_cancel = st.columns([1, 4])
            with col_exec:
                if st.button("🔥 确认批量删除", type="primary", key="kc_batch_del_exec"):
                    if not confirmed:
                        st.warning("请先勾选上方的确认复选框。")
                    else:
                        ...
                        if failed_cnt == 0:
                            st.success(f"✅ 已成功删除 {deleted} 个文档。")
                        else:
                            st.warning(f"⚠️ 部分文档删除失败：成功 {deleted} 个，失败 {failed_cnt} 个，请查看下方明细。")
...
def _render_cleanup_tab(admin_service: KnowledgeAdminService):
    ...
    st.markdown("---")
    st.markdown("### 🧹 一键清理失败导入记录")
    st.info(
        "**清理范围**：失败状态的 documents 记录、ChromaDB 残留向量、message_sources 脏引用、磁盘残留文件。\n\n"
        "**绝对不会清理**：已成功导入的文档、demo_documents 原始演示资料、数据库本身。"
    )
    ...
    if st.button("🧹 执行清理", type="primary", use_container_width=True, key="kc_clean_exec_btn"):
        if not confirmed_clean:
            ...
            if failed_cnt == 0:
                st.success(f"✅ 清理完成！共清理 {cleared} 条失败记录。")
            else:
                st.warning(f"⚠️ 清理部分完成：已清理 {cleared} 条，{failed_cnt} 条清理失败，请查看明细。")
